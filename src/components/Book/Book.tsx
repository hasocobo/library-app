      ...
      <Link to={`/browse/${bookElement.id}`}>
        <div className="md:h-100 h-100 rounded-sm border shadow-sm hover:shadow-md">
          <div id='image' className='h-40 md:h-80'>
            <img src={bookElement.imageUrl ? bookElement.imageUrl : bookImage} className='object-cover h-full w-full overflow-hidden' alt="" />
          </div>
          <div className='flex flex-col p-4'>
            <h3 className="lg:text-lg text-md font-semibold">{bookElement.title}</h3>
            <p className="text-sm font-semibold text-slate-500">
              {bookElement.authorName}
            </p>
            <p className="text-sm text-slate-500">
              {bookElement.publishYear}
            </p>
          </div>
        </div>
      </Link>
