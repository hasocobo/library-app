...
const AdminUserManagement = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await api.get('/users'); // Search and pagination params commented out
    ...
    fetchUsers();
  }, []); // Dependencies exclude search & page for now

  return (
    <div className="mx-auto max-w-7xl p-6">
      <h2 className="text-2xl font-semibold text-gray-800 mb-6">Kullanıcılar</h2>
            ...
            {users && users.length > 0 ? (
              users.map((user: TUser) => (
                <tr key={user.id} className="hover:bg-gray-50">
                  ...
                  <td className="border p-3">{user.roles.join(', ') || 'Yok'}</td>
